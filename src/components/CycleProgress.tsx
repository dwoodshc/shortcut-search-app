...
export default function CycleProgress(): React.JSX.Element {
  ...
  const daysElapsed = Math.floor((today.getTime() - cycleStart.getTime()) / MS_PER_DAY) + 1;
  const daysRemaining = CYCLE_LENGTH_DAYS - daysElapsed;
  const pctComplete = Math.min(100, Math.max(0, (daysElapsed / CYCLE_LENGTH_DAYS) * 100));
        ...
        <table className="w-full border-separate border-spacing-0 bg-white rounded-lg shadow-[0_2px_4px_rgba(0,0,0,0.08)] border border-[#F0F0F7]" style={{ borderCollapse: 'separate' }}>
          <thead>
            <tr className="bg-[#494BCB] text-white">
              ...
              <th className="px-2 py-2 text-center font-semibold text-[0.8rem]">Start</th>
          ...
          <tbody>
            <tr>
              ...
              <td className="px-3 py-2 text-center text-sm font-semibold text-[#1a202c] border-b border-[#F0F0F7]">{daysElapsed}</td>
              <td className="px-3 py-2 text-center text-sm font-semibold text-[#1a202c] border-b border-[#F0F0F7]">{daysRemaining}</td>
