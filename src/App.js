                        ...
                        <div style={{ display: 'flex', height: '22px', borderRadius: '999px', overflow: 'hidden', border: '1px solid #e2e8f0' }}>
                          {total === 0 ? (
                            ...
                            <>
                              ...
                              {inProgressPct > 0 && <div style={{ ...(backlogPct > 0 ? { width: `${inProgressPct}%`, clipPath: 'polygon(0 0, calc(100% - 7px) 0, 100% 50%, calc(100% - 7px) 100%, 0 100%)', marginRight: '-7px' } : { flex: 1 }), background: '#fde68a', height: '100%', minWidth: '2px', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', zIndex: 2 }}>{inProgressPct >= 8 && <span style={{ fontSize: '0.7rem', fontWeight: 700, color: '#92400e', whiteSpace: 'nowrap', paddingRight: backlogPct > 0 ? '7px' : '0' }}>{Math.round(inProgressPct)}%</span>}</div>}
                              {backlogPct > 0 && <div style={{ flex: 1, background: '#f1f5f9', height: '100%', minWidth: '2px', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', zIndex: 1 }}>{backlogPct >= 8 && <span style={{ fontSize: '0.7rem', fontWeight: 700, color: '#64748b', whiteSpace: 'nowrap' }}>{Math.round(backlogPct)}%</span>}</div>}
                            </>
                          )}
                        </div>
